...
class simple_GC_DEC(nn.Module):
    ...
    def fit(self, X, adj, lr=0.001, max_epochs=5000, update_interval=3, trajectory_interval=50,weight_decay=5e-4,opt="sgd",init="louvain",num_neighbors=10,res=0.4,num_clusters=10,init_spa=True,tol=1e-3):
        self.trajectory=[]
        if opt=="sgd":
        ...
        #----------------------------------------------------------------        
        if init=="kmeans":
            print("Initializing cluster centers with kmeans, num_clusters known")
            self.num_clusters=num_clusters
            kmeans = KMeans(self.num_clusters, n_init=20)
            if init_spa:
            ...
            self.num_clusters=len(np.unique(y_pred))
        #----------------------------------------------------------------
        y_pred_last = y_pred
        self.mu = Parameter(torch.Tensor(self.num_clusters, self.nhid))
